...
(function () {
  ...
  function PetitionForm({
    ...
    const submit = async e => {
      ...
      try {
        if (result && result.referral_code) localStorage.removeItem('ff_pending_signup');else safeSet('ff_pending_signup', JSON.stringify({
          first_name: d.firstName.trim(),
          last_name: d.lastName.trim(),
          email: d.email.trim(),
          mobile: d.mobile.trim(),
      ...
      try {
        window.location.assign(afterSignUrl(result));
        return;
      } catch (e2) {}
      setBusy(false);
    };
